...
int main() {
    ...
    while (window.isOpen()) {
        ...
        */
         ...
         */
        //03/19/2022
        //Updated boundaries for rocket to wrap around the screen
        //Ran into issues trying to smooth out the warp of the rocket around the screen
        //so the rocket is not appearing currently when running the program.
        if(rocketPosition.x < 0) rocketPosition.x = window.getSize().x;
        if(rocketPosition.x - (rocket.getTexture()->getSize().x * 0.5) > window.getSize().x){
            rocketPosition.x = 0 - rocket.getTexture()->getSize().x * 0.5;
        }
        if(rocketPosition.y < 0) rocketPosition.y = window.getSize().y;
        if(rocketPosition.y > window.getSize().y) rocketPosition.y = 0;
        //Updated boundaries for rocket to wrap around the screen
